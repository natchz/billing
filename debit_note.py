...
pathlib.Path('debit_notes/{}'.format(TODAY)).mkdir(parents=True, exist_ok=True)
NSMAP = {
...
def invoice_to_xml(invoice_id, client, items, issuer):
    ...
    etree.ElementTree(root).write("debit_notes/{}/{}".format(TODAY, file_name(issuer["nit"], invoice_id)),
                                  xml_declaration=True, encoding='UTF-8', standalone=False,
                                  pretty_print=True)
    sign_invoice("debit_notes/{}/{}".format(TODAY, file_name(issuer["nit"], invoice_id)))
    ws_zip_file("debit_notes/{}/".format(TODAY), file_name(issuer["nit"], invoice_id))
